package com.example.springbootxxljob.job;
...
public class SampleXxlJob {
    ...
    private String str = "XXL-JOB,参数:";
    private static ExecutorService pool;

    /**
     * 这里引入多线程
     * @PostConstruct 的作用是在项目启动时执行这个方法
     * maximumPoolSize 设置为 2 ，拒绝策略为 AbortPolicy 策略，直接抛出异常
     */
    @PostConstruct
    public void init() {
    ...
    public void demoJobHandler() {
        ...
        XxlJobHelper.log(str + command);
        String[] split = command.split("-");
        if (split.length != 2) {
            XxlJobHelper.log("参数错误---" + command);
        } else {
            Integer taskId = Integer.parseInt(split[0]);
            Integer jobId = Integer.parseInt(split[1]);
            /**
             * 处理业务的代码
             */
        }
